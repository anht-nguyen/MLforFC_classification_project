...
for metric in base_metrics:
    ...
    plt.tight_layout()
    img_path = os.path.join(out_dir, f"{metric}_all_models_bar_AO_cond.png")
...
best = sorted(best, key=lambda r: (r['Model'], r['FC']))
...
lines.append(r"\multicolumn{6}{c}{\tetbf{AO Condition Classification}}\\")
lines.append(r"\hline")
...
table_lines.append(r"\multicolumn{14}{c}{\textbf{AO Condition Classification}}\\")
...
for model_name in model_names:
    if model_name in ["CNN", "MLP", "GCN"]:
        ...
        meta_path = os.path.join(meta_dir, f"output_data-2025-{model_name}.json")
        if not os.path.isfile(meta_path):
            print(f"[WARN] meta JSON not found for {model_name}: {meta_path}")
            continue
    else:
        ckpt_dir = ml_ckpt_dir
        meta_path = os.path.join(meta_dir, f"output_data-2025-SK.json")
        if not os.path.isfile(meta_path):
            print(f"[WARN] meta JSON not found for {model_name}: {meta_path}")
            continue
...
for model_name in model_names:
    ...
    if model_name in ["CNN", "MLP", "GCN"]:
        ...
        meta_path = os.path.join(meta_dir, "output_data-2025-SK.json")
...
lines.append(" & ".join(hdrs) + r" \\")
lines.append(r"\hline")
lines.append(r"\multicolumn{5}{c}{\textbf{AO Condition Classification}}\\")
lines.append(r"\hline")
